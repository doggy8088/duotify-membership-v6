...
public sealed class VerifyEmailCodeServiceTests
{
    [Fact]
    public async Task VerifyAsync_ReturnsFailure_WhenRegistrationReferenceDoesNotExist()
    {
        var service = CreateService();

        var result = await service.VerifyAsync("missing", "123456", null, null, CancellationToken.None);

        Assert.False(result.Succeeded);
        Assert.Equal("registration_not_found", result.ErrorCode);
    }

    [Fact]
    public async Task VerifyAsync_ReturnsFailure_WhenNoCurrentChallengeExists()
    {
        ...
        var result = await service.VerifyAsync(member.RegistrationReference, "123456", null, null, CancellationToken.None);

        Assert.False(result.Succeeded);
        Assert.Equal("challenge_not_found", result.ErrorCode);
    }

    private static VerifyEmailCodeService CreateService(Member? member = null)
    {
        ...
        return new VerifyEmailCodeService(
            new FakeMemberRepository(member),
            new FakeChallengeRepository(),
            new FakeAuditRepository(),
            new SqlConnectionFactory(configuration),
            new VerificationCodeProtector(Microsoft.Extensions.Options.Options.Create(new VerificationCodeOptions
            {
    ...
    private sealed class FakeMemberRepository(Member? member) : IMemberRepository
    {
        public Task<Member?> GetByEmailAsync(string email, CancellationToken cancellationToken) => Task.FromResult<Member?>(null);
        public Task<Member?> GetByNationalIdAsync(string nationalId, CancellationToken cancellationToken) => Task.FromResult<Member?>(null);
        public Task<Member?> GetByRegistrationReferenceAsync(string registrationReference, CancellationToken cancellationToken)
            => Task.FromResult(member is not null && member.RegistrationReference == registrationReference ? member : null);
        public Task<Member?> GetByIdAsync(Guid memberId, CancellationToken cancellationToken) => Task.FromResult(member is not null && member.MemberId == memberId ? member : null);
